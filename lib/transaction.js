...
Transaction.decode = function(buf) {
  var doc = {};
  buf.cursor = 0;
  ...
  for (var i=0; i < inputCount; i++) {
    ...
    var txHashBytes = convBin(buf.slice(buf.cursor, buf.cursor+32), {out: 'bytes'})
    var txHash = txHashBytes.reverse()
    data.outpoint.hash = convBin(txHash).toString('hex')
    buf.cursor += 32
