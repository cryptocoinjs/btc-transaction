...
function (connectedScript, inIndex, hashType)
{
  ...
  return sha256.x2(buffer, {asBytes: true});
};
...
Transaction.prototype.getHash = function ()
{
  ...
  return sha256.x2(buffer, {asBytes: true}).reverse();
};
...
Transaction.deserialize = function(buf) {
  if (!Buffer.isBuffer(buf)) {
    buf = binConv(buf, {out: 'buffer'});
  }

  var doc = {};
  buf.cursor = 0;

  doc.version = buf.readUInt32LE(buf.cursor)
  advanceCursor(buf, 4)

  doc.ins = []
  doc.outs = []

  var inputCount = getVarInt(buf)
  for (var i=0; i < inputCount; i++) {
    var data = {}
    data.outpoint = {}

    data.outpoint.hash = binConv(binConv(buf.slice(buf.cursor, buf.cursor+32), {out: 'bytes'}).reverse(), {out: 'hex'})
    advanceCursor(buf, 32)

    data.outpoint.index = buf.readUInt32LE(buf.cursor)
    advanceCursor(buf, 4)

    var scriptLength = getVarInt(buf)
    data.script = new Script( binConv(buf.slice(buf.cursor, buf.cursor+scriptLength)).toString('hex') )
    advanceCursor(buf, scriptLength)

    data.sequence = buf.readUInt32LE(buf.cursor)
    advanceCursor(buf, 4)

    doc.ins.push(data)
  }

  var outputCount = getVarInt(buf)
  for (var i=0; i < outputCount; i++) {
    var data = {}
    data.value = binConv(buf.slice(buf.cursor, buf.cursor+8), {out: 'bytes'})
    advanceCursor(buf, 8)
  ...
  doc.lock_time = buf.readUInt32LE(buf.cursor)
  advanceCursor(buf, 4)
  return new Transaction(doc)
}

function getVarInt(buf) {
  var flag = buf[buf.cursor];
  ...
  if (flag < 0xfd) {
    ...
    advanceCursor(buf, 9)
    return buf.slice(offset+1, offset+9);
  }
};

function advanceCursor(buf, offset) {
  if (buf.cursor+offset > buf.length) {
    throw 'buffer EOF. Cursor: ' + (buf.cursor+offset) + ' Length: ' + buf.length
  }
  buf.cursor += offset
}
