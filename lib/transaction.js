...
Transaction.deserialize = function(buf) {
  ...
  for (var i=0; i < inputCount; i++) {
    ...
    data.outpoint.hash = buf.slice(buf.cursor, buf.cursor+32).toString('hex')
    ...
    data.script = new Script( convBin(buf.slice(buf.cursor, buf.cursor+scriptLength)).toString('hex') )
  ...
  for (var i=0; i < outputCount; i++) {
    var data = {}
    data.value = convBin(buf.slice(buf.cursor, buf.cursor+8), {out: 'bytes'})
    advanceCursor(buf, 8)
